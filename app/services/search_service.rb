class SearchService
  ...
  def textSearch_all(search_criteria)
    notes = textSearch_notes(search_criteria.query, search_criteria.limit)

    pages = textSearch_pages(search_criteria.query, search_criteria.limit)

    search_criteria.sort_by = "recent"
    profiles = profiles(search_criteria)

    tags = textSearch_tags(search_criteria.query, search_criteria.limit)

    maps = find_maps(search_criteria.query, search_criteria.limit)

    questions = textSearch_questions(search_criteria.query, search_criteria.limit)
  ...
  # Search pages??? and notes?? or a general node search?
  def textSearch_pages(input, limit = 25, order = :natural, type = :boolean)
    Node.search(query: input, order: order, type: type, limit: limit)
        .where("`node`.`type` = 'page' OR `node`.`type` = 'place' OR `node`.`type` = 'tool'")
  end

  # Search maps for matching text and package up as a DocResult
  def textSearch_maps(input, limit = 25, order = :natural, type = :boolean)
    Node.search(query: input, order: order, type: type, limit: limit)
        .where("`node`.`type` = 'map'")
  end
  ...
  def textSearch_questions(input, limit = 25, order = :natural, type = :boolean)
    Node.search(query: input, order: order, type: type, limit: limit)
        .where("`node`.`type` = 'note'")
        .joins(:tag)
